...
- Edit publications in `_bibliography/papers.bib`. Entries whose `additional_info` is `Preprint` are copied into the CV automatically. Do not edit the generated `Preprints` section in `_data/cv.yml` by hand.
